...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
    ...
    if coco_evaluator is not None:
        ...
        #     # Access the evaluation results for bbox
        #     bbox_eval = coco_evaluator.coco_eval['bbox']

        #     # Get the category IDs and names
        #     categories = base_ds.cats  # COCO categories from the dataset
        #     category_names = {cat_id: cat_info["name"] for cat_id, cat_info in categories.items()}
            
        #     # Extract precision (AP) for each class
        #     precisions = bbox_eval.eval['precision']  # [TxRxKxAxM]
        #     # Dimensions: T (IoU thresholds), R (recall thresholds), K (categories), A (area ranges), M (max detections)
            
        #     for idx, cat_id in enumerate(bbox_eval.params.catIds):
        #         class_name = category_names[cat_id]
        #         ap = precisions[:, :, idx, 0, -1].mean()  # Average over IoU thresholds, recall, and max detections
