...
do
    ...
    scaled_LR=$(awk "BEGIN {printf $random_LR * 0.0001 + 0.00005}")
    truncated_LR=$(awk 'BEGIN {printf "%.6f\n", '$scaled_LR'}')
    ...
    scaled_WD=$(awk "BEGIN {printf $random_WD * 0.0019 + 0.0001}")
